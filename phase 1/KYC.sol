...
contract KYC {
    ...
    uint totalBanks; //variable to keep track of totalBanks in the System
    ...
    function addBank(string memory _bankName, address _bankAddress, string memory _regNumber) public {
        ...
        require(banks[_bankAddress].ethAddress != _bankAddress, "the bank already exists");
    ...
    function isBankAllowedToVote(address _bankAddress, bool value) public {
        ...
        require(_bankAddress != address(0), "address of a bank cannot be 0");
        require(banks[_bankAddress].ethAddress == _bankAddress, "the bank does not exist exist");
    ...
    function removeBank(address _bankAddress) public {
        ...
        require(_bankAddress != address(0), "address of a bank cannot be 0");
        require(banks[_bankAddress].ethAddress == _bankAddress, "the bank does not exist");
    ...
    function addRequest(string memory _userName, string memory _customerData) public {
        require(requests[_userName].bankAddress == address(0), "Customer is already present in the KYC requests");
        require(banks[msg.sender].ethAddress != address(0), "only a bank can call the function");
    ...
    function addCustomer(string memory _userName, string memory _customerData) public {
        ...
        require(customers[_userName].bank == address(0), "Customer is already present in the customers' list, please call modifyCustomer to edit the customer data");
        require(requests[_userName].bankAddress != address(0), "Customer is not present in the KYC requests");
        require(banks[msg.sender].ethAddress != address(0), "only a bank can call the function");
    ...
    function upvoteCustomer(string memory _userName) public {
        ...
        require(!hasVotedforCustomer[_userName][msg.sender], "the bank has already voted for the customer");
        ...
        if(customers[_userName].upvotes + customers[_userName].downvotes == totalBanks 
    ...
    function getBankComplaints(address _bankAddress) public view returns(uint) {
        require(banks[_bankAddress].ethAddress != address(0), "bank does not exist");
        require(banks[msg.sender].ethAddress != address(0), "only a bank can call the function");
        
        return banks[_bankAddress].complaintsReported;
    }
    
    function viewBankDetails(address _bankAddress) public view returns(string memory, address, uint, uint, bool, string memory) {
        require(banks[_bankAddress].ethAddress != address(0), "bank does not exist");
        require(banks[msg.sender].ethAddress != address(0), "only a bank can call the function");
